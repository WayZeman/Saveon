...
export default function HomePageContent() {
  ...
  return (
    <div className="section-spacing max-w-6xl mx-auto">
      ...
      {/* Доходи та витрати — стиль Interactive Brokers: табличний підсумок + equity curve */}
      <section className="card opacity-0 animate-slide-up animate-stagger-5 overflow-hidden !rounded-lg border border-[var(--border-strong)]">
        <div className="border-b border-[var(--border)] px-4 py-3">
          <h2 className="text-[15px] font-semibold text-[var(--text)]">{t("home_incomeExpense")}</h2>
          <p className="text-[12px] text-[var(--text-tertiary)] mt-0.5">{t("home_incomeExpenseHint")}</p>
        </div>
        ...
        {data.monthlyData.length > 0 && (() => {
          ...
          return (
            <>
              {/* Табличний ряд як в IB */}
              <div className="grid grid-cols-3 gap-0 border-b border-[var(--border)]">
                <div className="px-4 py-3 border-r border-[var(--border)]">
                  <p className="text-[11px] text-[var(--text-tertiary)] uppercase tracking-wider mb-1">{t("home_income")}</p>
                  <p className="text-[15px] font-semibold tabular-nums text-[var(--accent-green)]">{formatMoney(totalIncome)}</p>
                </div>
                <div className="px-4 py-3 border-r border-[var(--border)]">
                  <p className="text-[11px] text-[var(--text-tertiary)] uppercase tracking-wider mb-1">{t("home_expense")}</p>
                  <p className="text-[15px] font-semibold tabular-nums text-[var(--text)]">{formatMoney(totalExpense)}</p>
                </div>
                <div className="px-4 py-3">
                  <p className="text-[11px] text-[var(--text-tertiary)] uppercase tracking-wider mb-1">{t("home_balanceChart")}</p>
                  <p className={`text-[15px] font-semibold tabular-nums ${totalBalance >= 0 ? "text-[var(--accent-green)]" : "text-[var(--accent-red)]"}`}>{formatMoney(totalBalance)}</p>
                </div>
              </div>

              {/* Графік equity curve — один акцент на балансі, чиста сітка */}
              <div className="p-3 bg-[var(--bg-secondary)]/50">
                <div className="h-52 md:h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={chartData} margin={{ top: 12, right: 12, left: 8, bottom: 8 }}>
                      <CartesianGrid strokeDasharray="0" stroke="var(--border)" vertical={false} horizontal={true} />
                      <XAxis
                        dataKey="month"
                        axisLine={true}
                        tickLine={false}
                      ...
                      />
                      <Tooltip
                        contentStyle={{ backgroundColor: "var(--bg-elevated)", border: "1px solid var(--border-strong)", borderRadius: "6px", fontSize: "12px", padding: "8px 12px" }}
                        formatter={(value: number, name: string) => [formatMoney(value), name]}
                        cursor={{ stroke: "var(--border-strong)", strokeWidth: 1 }}
                      />
                      <ReferenceLine y={0} stroke="var(--border-strong)" strokeDasharray="4 4" />
                      <Line type="monotone" dataKey="cumulativeExpense" stroke={PALE_RED} strokeWidth={1.5} dot={false} activeDot={{ r: 4, fill: PALE_RED }} name={t("home_expense")} isAnimationActive connectNulls />
                      <Line type="monotone" dataKey="cumulativeIncome" stroke="var(--accent-green)" strokeWidth={1.5} dot={false} activeDot={{ r: 4, fill: "var(--accent-green)" }} name={t("home_income")} isAnimationActive connectNulls />
                      <Line type="monotone" dataKey="balance" stroke="var(--accent-blue)" strokeWidth={2.5} dot={false} activeDot={{ r: 5, fill: "var(--accent-blue)", stroke: "var(--bg-elevated)", strokeWidth: 2 }} name={t("home_balanceChart")} isAnimationActive connectNulls />
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
                <div className="flex items-center gap-6 mt-3 pt-2 border-t border-[var(--border)]">
                  <span className="flex items-center gap-1.5 text-[11px] text-[var(--text-tertiary)]">
                    <span className="w-4 h-0.5 bg-[var(--accent-blue)]" /> {t("home_balanceChart")}
                  </span>
                  <span className="flex items-center gap-1.5 text-[11px] text-[var(--text-tertiary)]">
                    <span className="w-4 h-0.5 bg-[var(--accent-green)]" /> {t("home_income")}
                  </span>
                  <span className="flex items-center gap-1.5 text-[11px] text-[var(--text-tertiary)]">
                    <span className="w-4 h-0.5" style={{ backgroundColor: PALE_RED }} /> {t("home_expense")}
                  </span>
                </div>
              </div>
            </>
        ...
        {data.monthlyData.length === 0 && (
          <div className="px-4 py-8 text-center text-[var(--text-tertiary)] text-[13px] border-t border-[var(--border)]">
            {t("home_noDataPeriod")}
          </div>
        )}
      </section>
